...
class DiskBlaster(navis.nbl.nblast_funcs.NBlaster):
    ...
    def multi_query_target_disc(self, q_idx, t_idx,
                                chunksize=10,
                                scores='forward',
                                smart=False):
        """BLAST multiple queries against multiple targets - on disk.
...
def nblast_disk(query: Union[Dotprops, NeuronList],
    ...
    if n_cores == 1:
        _ = this.multi_query_target_disc(this.queries,
                                         this.targets,
                                         smart=smart,
                                         scores=scores)
    else:
        with ProcessPoolExecutor(max_workers=n_cores) as pool:
            ...
            futures = [pool.submit(this.multi_query_target_disc,
                                   q_idx=this.queries,
                                   t_idx=this.targets,
                                   smart=smart,
